...
// 👉 Track escalations (Track ongoing human escalations)
const activeHumanChats = new Map<string, boolean>(); // Stores userId -> true
...
export const socketHandler = (io: Server) => {
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("send_message", async (data: SendMessagePayload) => {
      ...
      const actualSender = sender || "user";

      // 1. Save User/Admin message to DB
      await Message.create({ userId, sender:actualSender, message });
      ...
      // 5. AI Logic (Only if no human is active)
      const aiResult = await generateGeminiResponse(message, userId);

      if (isManual || aiResult.needsEscalation) {
        activeHumanChats.set(userId, true);
        // Alert all connected admins
        io.emit("admin_alert", { userId, reason: aiResult.reason || "User requested human",});
